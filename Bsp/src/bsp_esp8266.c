...
void wifi_auto_detected_link_state(void)
{
	...
	if(gpro_t.wifi_connected_success_flag ==0 && gpro_t.link_net_flag ==0){
		
       auto_connect_wifi_handler();//InitWifiModule();
	
	 
    }
	else if(gpro_t.wifi_connected_success_flag==1 && link_counter_times ==0){
              
           link_counter_times ++ ;
		   link_counter_times =10;
           gpro_t.wifi_linking_tencent_f =0;//wifi_t.linking_tencent_cloud_doing = 0;
   ...
   if(gpro_t.gTime_link_net_counter > 4 && link_counter_times ==1){
	  gpro_t.gTime_link_net_counter =0;
...
//static uint8_t link_wifi_f=0;
  ...
  switch(dc_connect_net_step){
	...
	case 0:
	    ...
	    wifi_rx_numbers = 0;
        gpro_t.wifi_linking_tencent_f =1;//linking_tencent_cloud_doing = 1;
	...
	case 2:
     ...
     if(time_autolink_counter >4){
        ...
        time_autolink_counter=0;
		 gpro_t.wifi_linking_tencent_f = 1;//net_t.linking_tencent_cloud_doing  = 1;
	     wifi_rx_numbers = 0; 
	...
	case 3:
	 ...
	 case 4:
	   ...
	   if(gpro_t.wifi_connected_success_flag==1 && time_autolink_counter > 2){
	    ...
	    else if(gpro_t.wifi_connected_success_flag==0 && time_autolink_counter > 2){
			time_autolink_counter=0;
			gpro_t.wifi_linking_tencent_f =0;
	       dc_connect_net_step=5;
	       if(gpro_t.g_power_flag==1){
...
/**********************************************************************
	**
	*Function Name : void wifi_power_on_handler(void)
	*Function: 
	*Input Ref: 
	*Return Ref:NO
	*
...
void wifi_power_on_handler(void)
{
   // static uint8_t sw_flag=0;
	...
	if(gpro_t.link_net_flag == 1 || gpro_t.g_power_flag == 0 || gpro_t.wifi_connected_success_flag==0) return ;
	 ...
	 case 0:
	   ...
	   else if(gpro_t.wifi_connected_success_flag ==1 && wifi_app_timer_power_on_f ==0){
	 ...
	 case 5: 
	   ...
	   counter_1 ++;
        ...
        if(gpro_t.wifi_connected_success_flag ==1 &&  wifi_app_timer_power_on_f ==0 &&  counter_1 > 3){
	 ...
	 case 6:
		counter++;
         if(gpro_t.wifi_connected_success_flag ==1 &&  wifi_app_timer_power_on_f ==0 && wifi_first_connectoed_cloud_f==0){
	...
	break;
	 ...
	 case 7:
		...
		if(gpro_t.wifi_connected_success_flag ==1  && soft_version == 0){ //WT.EDIT 2026.02.27
		...
		else if(gpro_t.wifi_connected_success_flag ==0 && counter > 1 && soft_version ==0){ //WT.EDIT 2026.02.27
			counter =0;
			sw_flag = sw_flag ^ 0x01;
			if(sw_flag == 1){
				if(gpro_t.g_power_flag == 1){
	 ...
	 break;
      ...
      case 9:
	  	  send_times ++ ;
	  	  if(gpro_t.wifi_connected_success_flag ==1 &&  send_times > 4){
		  	    send_times=0;
			
				Update_Dht11_Totencent_Value();
                 wait_timeout=tx_time_get()+10;//tx_thread_sleep(20);// delay_ms(200);
        }
	  ...
	  static uint8_t  counter=0,sw_flag=0;
	   ...
	   switch(wifi_off_step){
		...
		case 2: 
		   ...
		   if(gpro_t.wifi_connected_success_flag ==1 && gpro_t.time_7s_f > 1){
